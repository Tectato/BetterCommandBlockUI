...
public class BetterCommandBlockScreen extends AbstractBetterCommandBlockScreen {
        ...
        if(chainHandler.isInChain()){
            ...
            if(next != null && next.getBlock().equals(Registries.BLOCK.get(Identifier.of("minecraft","chain_command_block")))){
                ...
                chainNext = this.addDrawableChild(
                        new TexturedButtonWidget( 49, 5, 20, 20, 0, 20, 20, BlockStateToButtonTexture(next), 20, 60, (button -> {
                            moveAlongChain(dir);
                        }), Text.of(""))
                );
            ...
            if(!prior.isEmpty()){
                ...
                for(Pair<BlockState,Direction> entry : prior){
                    chainPrior.add(this.addDrawableChild(
                            new TexturedButtonWidget( 5, y, 20, 20, 0, 20, 20, BlockStateToButtonTexture(entry.getLeft()), 20, 60, (button -> {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if(chainHandler.isInChain()){
            ...
            Identifier texture = BlockStateToButtonTexture(blockState);
            context.drawTexture(texture, 27, 5, 0, 20, 20, 20, 20, 60);
